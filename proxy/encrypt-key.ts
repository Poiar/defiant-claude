...
// Reads DEFIANT_ENCRYPTION_KEY from environment.
//
...
async function main(): Promise<void> {
  ...
  if (process.argv[2]) {
    key = process.argv[2];
    console.warn(
      'Warning: passing the key as a command-line argument exposes it in the process list. Consider using stdin instead.',
    );
  } else {
  ...
  const masterSecret = process.env.DEFIANT_ENCRYPTION_KEY;

  if (!masterSecret) {
    console.error('Error: DEFIANT_ENCRYPTION_KEY environment variable is not set');
    process.exit(1);
  }
  ...
  if (masterSecret.length < 32) {
    console.warn(
      'Warning: DEFIANT_ENCRYPTION_KEY is shorter than 32 characters. This is acceptable (scrypt stretches it), but a longer key is recommended.',
    );
  }
