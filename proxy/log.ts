...
const LOG_DIR = path.join(os.homedir(), '.defiant');
...
const DEBUG_ENABLED: boolean =
  ['true', '1', 'yes'].includes((process.env.DEFIANT_DEBUG || '').toLowerCase()) ||
  ['true', '1', 'yes', 'debug'].includes((process.env.DEFIANT_LOG_LEVEL || '').toLowerCase());
