...
function writeSubagentModel(dir: string, data: unknown): void {
  const subDir = path.join(dir, '.defiant');
...
function _removeSubagentModel(dir: string): void {
  const filePath = path.join(dir, '.defiant', 'subagent-model.json');
  try {
...
describe('resolveSubagentModel', () => {
  ...
  test('returns null for invalid JSON content', () => {
    const subDir = path.join(tmpDir, '.defiant');
    fs.mkdirSync(subDir, { recursive: true });
...
describe('resolveTarget with dedicated subagent model', () => {
  ...
  test('invalid subagent model file falls back to config route', async () => {
    // Write invalid JSON
    fs.mkdirSync(path.join(tmpDir, '.defiant'), { recursive: true });
    fs.writeFileSync(path.join(tmpDir, '.defiant', 'subagent-model.json'), '{{{broken');
