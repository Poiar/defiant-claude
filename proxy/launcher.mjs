...
function keyEnvToShortName(keyEnv) {
    const map = {
        ...
        MIMO_API_KEY: 'mm', UMANS_API_KEY: 'um', GROQ_API_KEY: 'gr',
        MISTRAL_API_KEY: 'mt', MINIMAX_API_KEY: 'mx', ZAI_API_KEY: 'za',
        BYTEPLUS_API_KEY: 'bp', SILICONFLOW_API_KEY: 'sf', NOVITA_API_KEY: 'nv',
        GROK_API_KEY: 'gk',
    };
    return map[keyEnv] || '';
}
...
function adhocSlotIndex(specCount, slotIndex) {
    ...
    switch (specCount) {
        case 1: return 0;
        case 2: return slotIndex < 3 ? 0 : 1;
        case 3: return slotIndex === 0 ? 0 : (slotIndex <= 2 ? 1 : 2);
        case 4: return slotIndex < 3 ? slotIndex : 3;
        default: return slotIndex;
    }
}
...
function resolveConfig(configName) {
    ...
    const resolved = { name: cfg.name, slots: {}, modelProviders: {}, providers: {}, defaultProvider: '' };
    for (const slot of SLOTS) {
        ...
        if (!resolved.providers[provKey]) {
            ...
            resolved.providers[provKey] = {
                name: prov.displayName, url: prov.endpoint, keyEnv: prov.keyEnv,
                auth: prov.authHeader || 'bearer', format: prov.wireFormat || 'anthropic',
                key: getKey(provKey),
            };
            ...
            if (prov.streamUsageReporting) resolved.providers[provKey].streamUsageReporting = prov.streamUsageReporting;
        }
    }
...
function buildRoutesJson(resolved, includeAllModels = true) {
    ...
    for (const [provKey, info] of Object.entries(resolved.providers)) {
        ...
        const entry = {
            url: info.url, keyEnv: info.keyEnv, auth: info.auth,
            format: info.format || 'anthropic', fallback: fb,
        };
        ...
        for (const cfg of Object.values(reg.configs)) {
            for (const slot of SLOTS) {
                ...
                if (key && !providerEntries[provKey]) {
                    ...
                    const entry = {
                        url: prov.endpoint, keyEnv: prov.keyEnv, auth: prov.authHeader || 'bearer',
                        format: prov.wireFormat || 'anthropic', fallback: fb,
                    };
...
function setSlotOverride(slotName, slotModel) {
    ...
    overrides[slotName] = slotModel;
    writeOverridesFile(overrides);
    return { set: true, slot: slotName, value: slotModel, overrides };
}
...
function setSubagentModel(model) {
    if (!model) {
        try { rmSync(SUBMODEL_FILE, { force: true }); } catch {}
        return { cleared: true };
    }
...
function proxyStateCheck() {
    ...
    try {
        ...
        if (!alive) {
            try { rmSync(PROXY_STATE_FILE, { force: true }); } catch {}
...
function saveProxyState(pid, port, routesFile) {
    mkdirSync(DEEPCLAUDE_DIR, { recursive: true, mode: 0o700 });
    const state = { pid: Number(pid), port: Number(port), routesFile, startedAt: new Date().toISOString() };
    ...
    for (let retry = 0; retry < maxRetries; retry++) {
        try {
            if (existsSync(lockFile)) {
                ...
                let stale = true;
                if (lockPid > 0) {
                    try { process.kill(lockPid, 0); stale = false; } catch {}
                }
                if (stale) {
...
function doctorJson(configName) {
    ...
    for (const pk of pkOrder) {
        const prov = reg.providers[pk];
        const key = getKey(pk);
        keys[pk] = { keyName: prov.keyEnv, hasKey: !!key, masked: maskKey(key), displayName: prov.displayName };
        if (key) configured++;
    }
...
async function main() {
    ...
    const action = args[0];
    if (!action) fail('Usage: node launcher.mjs <action> [--key=value...]\nActions: ' +
    ...
    try {
        ...
        switch (action) {
            ...
            case 'build-routes': {
                ...
                else if (opts['config-file']) resolved = JSON.parse(readFileSync(opts['config-file'], 'utf-8'));
            ...
            case 'read-override': {
                ...
                result = { slot: opts.slot, value: append1m(opts.slot + ':' + value.split(':').slice(-1)[0]) };
                break;
            }
            case 'set-slot': {
            ...
            case 'env-vars': {
                ...
                result = computeEnvVars(
                    opts.port,
                    opts.opus || '', opts.sonnet || '', opts.haiku || '',
                    opts.subagent || '', opts.fable || '',
            ...
            case 'version':
                ...
                break;
            case 'write-atomic': {
                if (!opts.file || opts.data === undefined) fail('write-atomic requires --file=PATH --data=JSON');
            ...
            default:
                fail(`Unknown action '${action}'. Actions: resolve-config build-routes init-overrides read-override set-slot thinking-overrides subagent-model proxy-state proxy-save env-vars context-info cost-data model-list config-list key-status version write-atomic doctor-json`);
        }
