...
let subagentModelCache: {
  model: { providerKey: string; modelId: string };
  mtime: number;
  size: number;
} | null = null;
...
export function resolveSubagentModel(): { providerKey: string; modelId: string } | null {
  ...
  try {
    ...
    if (
      subagentModelCache &&
      subagentModelCache.mtime === stat.mtimeMs &&
      subagentModelCache.size === stat.size
    ) {
    ...
    if (parsed && typeof parsed.providerKey === 'string' && typeof parsed.modelId === 'string') {
      subagentModelCache = {
        model: { providerKey: parsed.providerKey, modelId: parsed.modelId },
        mtime: stat.mtimeMs,
        size: stat.size,
      };
      return subagentModelCache.model;
...
export async function resolveTarget(
  ...
  if (!provider || !providerKey) {
    return {
      error: providerKey ? 'Unknown provider: ' + providerKey : 'No default provider configured',
    };
  }
  ...
  async function resolveFallback(fbKey: string, fb: ProviderEntry): Promise<ResolvedTarget | null> {
    ...
    if (!fbRawKey) {
      log.warn(
        null,
        `Fallback provider "${fbKey}" has no API key (keyEnv=${fb.keyEnv || 'none'}) — skipping`,
      );
      return null;
    }
    ...
    if (fbRawKey.startsWith('$aes256gcm:') && fbResolvedKey === null) {
      log.warn(
        null,
        `Fallback provider "${fbKey}" has encrypted key but decryption failed — skipping`,
      );
      return null;
    }
    ...
    if (tierPart && routing.routes) {
      for (const [routeModel, routeEntry] of Object.entries(routing.routes)) {
        ...
        if (typeof routeEntry === 'string' && routeEntry === fbKey) {
          ...
          (routeEntry as { provider: string }).provider === fbKey
        ) {
          fbRewrite = (routeEntry as { rewrite?: string }).rewrite || routeModel;
          break;
        }
      }
    }
  ...
  if (provider.fallback && Array.isArray(provider.fallback)) {
    for (const fbKey of provider.fallback) {
      ...
      if (!fb) {
        log.warn(
          null,
          `Fallback provider "${fbKey}" not found in providers table (referenced by ${primary.providerKey}) — skipping`,
        );
        continue;
      }
  ...
  const hasNoAutoFallback =
    primaryDef?.noAutoFallback || HARDCODED_NO_AUTO_FALLBACK.has(primary.providerKey);
  if (fallbacks.length === 0 && routing.providers && !hasNoAutoFallback) {
    const autoFbKeys: string[] = [];
    for (const [fbKey, fb] of Object.entries(routing.providers)) {
      ...
      const fbTarget = await resolveFallback(fbKey, fb);
      if (fbTarget) {
        fallbacks.push(fbTarget);
        autoFbKeys.push(fbKey);
      }
    }
    if (autoFbKeys.length > 0) {
      log.info(
        null,
        `Auto-fallback: primary "${primary.providerKey}" has no explicit fallbacks — added ${autoFbKeys.length} auto-fallback(s): ${autoFbKeys.join(', ')}`,
      );
    }
  }
  ...
  if (!isProviderHealthy(primary.providerKey)) {
    if (fallbacks.length > 0) {
      ...
      return {
        error: 'Primary provider ' + primary.providerKey + ' is unhealthy (circuit breaker open)',
      };
    }
  }
