...
export async function runStartupChecks(): Promise<StartUpCheckSummary> {
  ...
  const probePromises = providerKeys.map((key) => {
    ...
    return Promise.all([
      sendProbe(key, pName, def.endpoint, apiKey, pAuth, pFmt, def.extraHeaders, def.noAuth),
      sendProbeStream(key, pName, def.endpoint, apiKey, pAuth, pFmt, def.extraHeaders, def.noAuth),
    ]);
  });
  ...
  for (const r of results) {
    if (r.errorSummary === 'NO KEY') {
      log.info(
        null,
        '  ' + r.providerKey.padEnd(4) + r.displayName.padEnd(30) + 'SKIP  (no API key configured)',
      );
      console.log(
        '  ' + r.providerKey.padEnd(4) + r.displayName.padEnd(30) + 'SKIP  (no API key configured)',
      );
      ...
      log.info(
        null,
        '  ' +
          r.providerKey.padEnd(4) +
          r.displayName.padEnd(30) +
          label +
          ' (stream verified)  ' +
          msStr,
      );
  ...
  if (allDown) {
    ...
    console.log(
      '  ⚠  All ' + providerKeys.length + ' providers unreachable — check API keys and network',
    );
  } else if (!allHealthy) {
    const parts: string[] = [];
    ...
    const failedNames = results
      .filter((r) => !r.success && r.errorSummary !== 'NO KEY')
      .map((r) => r.providerKey)
      .join(', ');
