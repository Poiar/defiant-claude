...
async function webFetchImpl(url: string, depth: number = 0, visited: Set<string> = new Set()): Promise<string> {
    ...
    return new Promise((resolve) => {
        ...
        const req = transport.request(requestOptions, (res) => {
            ...
            res.on('error', (err: Error) => resolve(`Web fetch failed: ${scrubCredentials(err.message)}. URL was: ${scrubCredentials(url)}`));
        });
        // FIX 1: Scrub credentials from URL in error messages
        req.on('error', (err: Error) => resolve(`Web fetch failed: ${scrubCredentials(err.message)}. URL was: ${scrubCredentials(url)}`));
        req.on('timeout', () => { req.destroy(); resolve(`Web fetch timed out for URL: ${scrubCredentials(url)}`); });
