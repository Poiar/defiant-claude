...
- **Thinking block management** (`proxy/thinking-cache.ts`, `proxy/reasoning-cache.ts`): For OpenAI-format providers, reasoning/thinking blocks are extracted from responses and cached for re-injection on subsequent turns (SDKs strip them across multi-turn tool conversations). For Anthropic-format providers like DeepSeek direct (`ds` via `/anthropic`), thinking blocks flow natively without stripping. Both caches use backward scan, DJB2 fingerprinting, and message count guards.
