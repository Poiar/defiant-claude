# Manifest Features Missing from Defiant — Prioritized Improvements
...
**What Defiant has**: 381-line embedded HTML dashboard (SSE every 2s). Shows health + recent requests + spend. No historical analytics.
...
**What Defiant has**: Shows spend. No comparison baseline.
...
**What Defiant has**: `requests.log` JSONL file (viewable via `--logs`/`--tail`). No UI for browsing.
...
**What Defiant has**: Single API key per provider in `providers.json` or env var. No agent concept. Single-user only.

**Why it matters**: Defiant is fundamentally single-user local. This is a design choice, not a gap — adding multi-tenancy would be a different product. However, encrypted API key storage with a master secret is already partially implemented.

**Suggested approach**: Not recommended for now. Defiant's single-user local model is simpler and appropriate for its use case. If team features are ever needed, consider the Manifest model.
...
**What Defiant has**: `crypto.ts` with AES-256-GCM + scrypt. `encrypt-key.ts` CLI tool. Manual setup required.

**Why it matters**: The encryption exists but requires manual CLI steps. Most users probably store keys in plaintext env vars.

**Suggested approach**:
1. On startup, check if `DEFIANT_MASTER_SECRET` is set
...
**What Defiant has**: OpenAI models only via OpenRouter. No direct OpenAI API.

**Why it matters**: OpenAI is the largest provider. Users with OpenAI API keys can't use them directly — they must go through OpenRouter (which adds latency and cost). This is Defiant's most glaring provider gap.
...
**What Defiant has**: No local model support. All providers are cloud APIs.
...
**What Defiant has**: API keys only. No subscription provider support.
...
**Suggested approach**: Not recommended for now. OAuth flows are complex, require browser interaction, and the subscription provider APIs are undocumented/internal. Defiant's target user has API keys.
...
**What Defiant has**: No xAI support.
...
**What Defiant has**: Static `providers.json` — manual updates required.
...
**What Defiant has**: 5 simple categories (TRIVIAL/CHAT/CODE/TOOL/HEAVY) based on character count + code block detection + tool count.

**Why it matters**: Better scoring = better routing = more cost savings without quality loss. Defiant's scoring is adequate but coarse.
...
**What Defiant has**: No task-type detection.

**Why it matters**: Different tasks benefit from different models. A coding-optimized model for code, a reasoning model for analysis. Defiant's slot-based routing partially addresses this but doesn't adapt to task type within a slot.
...
**What Defiant has**: No header-based routing. Routing is configured statically or via prompt analysis.
...
**Suggested approach**: Not a priority — Claude Code's slot system already provides this capability. Could be useful if Defiant ever serves non-CC clients.
...
**What Defiant has**: No Docker support. Local Node.js only.

**Why it matters**: Docker is the standard deployment method. Users on Linux/Mac who don't have Node.js can't easily run Defiant. A Docker image would also enable cloud/VPS deployment.

**Suggested approach**:
1. Create a simple two-stage Dockerfile (build → slim runtime)
2. No database needed — just Node.js + proxy
3. Expose the proxy port
4. Mount `~/.defiant` for config persistence
...
**What Defiant has**: Manual `npm install` + `npm link` setup. No installer.

**Why it matters**: Lower barrier to entry. Currently requires Node.js, npm, git clone, npm install, npm link.

**Suggested approach**: Write an install script that: checks Node.js >= 18, clones repo, runs npm install, links `defiant`/`dc` to PATH. Or publish to npm.
...
**What Defiant has**: Not published to npm. Manual clone + link.

**Why it matters**: `npm install -g defiant` would be dramatically simpler than the current setup. This is how CLI tools are distributed in the Node ecosystem.
...
**What Defiant has**: Manual version bumps.
...
**What Defiant has**: Budget caps (`DEFIANT_DAILY_BUDGET`, `--max-spend`) that reject requests. No notifications.
...
**Suggested approach**:
1. Add configurable warning thresholds (already partially implemented via `DEFIANT_BUDGET_WARNING`)
...
**What Defiant has**: No email capability.
...
**What Defiant has**: Integration test (`integration.test.ts`) that tests the proxy end-to-end. Good but limited.
...
**What Defiant has**: Jest coverage (unpublished).
...
**What Defiant has**: ~100 line CLAUDE.md with key files, test commands, and quick reference. Good but not comprehensive.
...
**What Defiant has**: None of these.
...
**What Defiant has**: `config-lint.ts` CLI tool (444 lines, 7 validation categories). Excellent but CLI-only.
...
**What Defiant has**: Env vars documented via `--help` and inline code. No `.env.example`.

**Why it matters**: Users setting up Defiant for the first time need to discover what env vars are available without reading source code.

**Suggested approach**: Create `.env.example` with all `DEFIANT_*` vars, grouped: required, provider keys, budget, behavior, debugging.
...
**What Defiant has**: Manual config file editing. `--doctor` checks setup but doesn't guide it.

**Why it matters**: Lower barrier for first-time users. But Defiant's audience is technical.

**Suggested approach**: Add `defiant --setup` with interactive prompts for: provider API keys, slot model choices, budget limits. Writes `providers.json` and `slot-overrides.json`.
...
**What Defiant has**: SSE at `/health/stream` (dashboard only). No external event bus.

**Why it matters**: Would allow external monitoring tools to consume Defiant events. Not critical — Prometheus metrics already serve this need.
...
## What Defiant Should NOT Adopt from Manifest

These are Manifest features that are intentionally out of scope:

1. **Multi-tenant architecture** — Defiant is a local single-user tool. Adding users/tenants/agents would make it a different product.
2. **PostgreSQL/TypeORM** — File-based persistence is simpler and appropriate for local use. No DB to manage.
3. **NestJS framework** — Manifest is a server; Defiant is a CLI-spawned proxy. The zero-dependency approach is correct.
...
6. **OTLP ingestion** — Defiant doesn't collect agent telemetry; it's the telemetry source (Prometheus).
