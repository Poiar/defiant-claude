...
describe('validateConfig', () => {
  ...
  test('circular fallback detection', () => {
    ...
    try {
      const issues = validateConfig(filePath);
      const circular = issues.filter(
        (i) => i.section === 'fallbacks' && i.message.indexOf('Circular') >= 0,
      );
  ...
  test('self-referencing fallback detection', () => {
    ...
    try {
      const issues = validateConfig(filePath);
      const selfRef = issues.filter(
        (i) => i.section === 'fallbacks' && i.message.indexOf('self-referencing') >= 0,
      );
  ...
  test('missing fallback target', () => {
    ...
    try {
      ...
      const missing = issues.filter(
        (i) => i.section === 'fallbacks' && i.message.indexOf('not found') >= 0,
      );
  ...
  test('invalid provider key in config', () => {
    ...
    try {
      const issues = validateConfig(filePath);
      const provErrors = issues.filter(
        (i) => i.section === 'configs' && i.message.indexOf('nonexistent') >= 0,
      );
  ...
  test('missing provider required fields', () => {
    ...
    try {
      ...
      const fieldIssues = issues.filter(
        (i) => i.section === 'providers' && i.message.indexOf('missing required field') >= 0,
      );
  ...
  test('invalid slot format in config', () => {
    ...
    try {
      ...
      const formatIssues = issues.filter(
        (i) => i.section === 'configs' && i.message.indexOf('invalid format') >= 0,
      );
...
describe('formatLintResults', () => {
  test('formats empty issues', () => {
    ...
    expect(output).toContain('Defiant Claude Config Lint');
