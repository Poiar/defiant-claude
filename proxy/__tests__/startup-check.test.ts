...
jest.mock('https', () => {
  ...
  return {
    request: jest.fn((_options: unknown, callback: Function) => {
      ...
      (mockRes as Record<string, unknown>).headers = {
...
describe('runStartupChecks', () => {
  ...
  beforeAll(() => {
    ...
    origEnv.DEFIANT_SKIP_STARTUP_CHECK = process.env.DEFIANT_SKIP_STARTUP_CHECK;
  ...
  afterAll(() => {
    if (origEnv.DEEPSEEK_API_KEY !== undefined)
      process.env.DEEPSEEK_API_KEY = origEnv.DEEPSEEK_API_KEY;
    else delete process.env.DEEPSEEK_API_KEY;
    if (origEnv.OPENROUTER_API_KEY !== undefined)
      process.env.OPENROUTER_API_KEY = origEnv.OPENROUTER_API_KEY;
    else delete process.env.OPENROUTER_API_KEY;
    if (origEnv.GROQ_API_KEY !== undefined) process.env.GROQ_API_KEY = origEnv.GROQ_API_KEY;
    else delete process.env.GROQ_API_KEY;
    if (origEnv.DEFIANT_SKIP_STARTUP_CHECK !== undefined)
      process.env.DEFIANT_SKIP_STARTUP_CHECK = origEnv.DEFIANT_SKIP_STARTUP_CHECK;
    else delete process.env.DEFIANT_SKIP_STARTUP_CHECK;
  });
  ...
  beforeEach(() => {
    ...
    delete process.env.DEFIANT_SKIP_STARTUP_CHECK;

    // Mock fs.readFileSync to return controlled providers.json data
    fsReadFileSyncSpy = jest
      .spyOn(fs, 'readFileSync')
      .mockImplementation((filepath: fs.PathOrFileDescriptor, ..._args: unknown[]) => {
  ...
  test('DEFIANT_SKIP_STARTUP_CHECK env var bypasses the check', async () => {
    process.env.DEFIANT_SKIP_STARTUP_CHECK = 'true';
