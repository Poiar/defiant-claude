...
export const ERROR_CODES: Record<string, ErrorCodeEntry> = {
  AUTH_FAILED: {
    code: 'AUTH_FAILED',
    status: 401,
    message: 'Upstream provider rejected credentials',
    ecode: 'E001',
    suggestion:
      "Check that your API key is set correctly in your environment variables. For DeepSeek, set DEEPSEEK_API_KEY. Get a key at the provider's API key page.",
  },
  FORBIDDEN: {
    code: 'FORBIDDEN',
    status: 403,
    message: 'Access denied by upstream provider',
    ecode: 'E002',
    suggestion:
      "Your API key does not have access to this resource. Verify the key has the correct permissions at your provider's dashboard.",
  },
  NOT_FOUND: {
    code: 'NOT_FOUND',
    status: 404,
    message: 'Resource not found on upstream provider',
    ecode: 'E003',
    suggestion:
      'The requested model or endpoint was not found. Verify the model name is correct and available on this provider.',
  },
  UPSTREAM_TIMEOUT: {
    code: 'UPSTREAM_TIMEOUT',
    status: 408,
    message: 'Request to upstream provider exceeded time limit',
    ecode: 'E004',
    suggestion:
      'The upstream provider took too long to respond. The proxy retries with fallback providers automatically. If this persists, check your network connection.',
  },
  RATE_LIMITED: {
    code: 'RATE_LIMITED',
    status: 429,
    message: 'Too many requests to upstream provider',
    ecode: 'E005',
    suggestion:
      "You've hit the provider's rate limit. The proxy will retry with a fallback provider. To avoid this, consider adding additional providers or API keys.",
  },
  UPSTREAM_ERROR: {
    code: 'UPSTREAM_ERROR',
    status: 502,
    ...
    ecode: 'E006',
    suggestion:
      "The upstream provider returned an error (HTTP {status}). Check your provider's status page. The proxy will try fallback providers automatically.",
  },
  UPSTREAM_UNAVAILABLE: {
    code: 'UPSTREAM_UNAVAILABLE',
    status: 503,
    message: 'Upstream provider is not reachable right now',
    ecode: 'E007',
    suggestion:
      'The upstream provider is not reachable. This may be a temporary outage. The proxy will try fallback providers automatically.',
  },
  GATEWAY_TIMEOUT: {
    ...
    ecode: 'E008',
    suggestion:
      'The upstream provider did not respond in time. This is usually temporary. The proxy retries with fallback providers.',
  },
  STREAM_DEAD: {
    code: 'STREAM_DEAD',
    status: 502,
    message: 'Provider returned 200 but stream produced no data',
    ecode: 'E009',
    suggestion:
      'The provider accepted the request but produced no response data. This often happens with overloaded providers. The proxy will retry with a fallback.',
  },
  BODY_TOO_LARGE: {
    code: 'BODY_TOO_LARGE',
    status: 413,
    ...
    ecode: 'E010',
    suggestion:
      'Your request body is too large. Reduce the context size or split your request into smaller parts.',
  },
  UNKNOWN_PROVIDER: {
    ...
    ecode: 'E011',
    suggestion:
      "The provider '{provider}' is not configured. Check your config with 'defiant --status' or switch to a known config with 'defiant --switch'.",
  },
  ALL_PROVIDERS_FAILED: {
    ...
    ecode: 'E012',
    suggestion:
      "All configured providers failed. Check your API keys with 'defiant --doctor' and verify your network connection.",
  },
  NO_DEFAULT_PROVIDER: {
    ...
    ecode: 'E013',
    suggestion:
      "No default provider is configured. Set up a provider with 'defiant --switch <config>' or set DEFIANT_DEFAULT_BACKEND.",
  },
  BUDGET_EXCEEDED: {
    code: 'BUDGET_EXCEEDED',
    status: 402,
    message: 'Budget limit reached: {reason}',
    ecode: 'E014',
    suggestion:
      "Your usage has reached the configured budget cap. Adjust the cap with 'defiant --set-budget', check your daily limit, or wait until the budget resets.",
  },
};
...
export const STATUS_TO_CODE: Record<number, ErrorCodeEntry> = {
  ...
  402: ERROR_CODES.BUDGET_EXCEEDED,
...
export function formatError(
  ...
  const entry = STATUS_TO_CODE[status] || {
    ...
    ecode: 'E006',
    suggestion:
      'The upstream provider returned an error (HTTP ' +
      status +
      "). Check your provider's status page. The proxy will try fallback providers automatically.",
  };
...
export function formatExhaustedError(
  ...
  if (qualityReason) {
    base.message =
      'All configured providers failed (last error: ' + scrubCredentials(qualityReason) + ')';
  } else {
    base.message =
      'All configured providers failed' + (lastStatus ? ' (last error: ' + lastStatus + ')' : '');
  }
...
const CREDENTIAL_PATTERNS: CredentialPattern[] = [
  ...
  [
    /"(api_key|apiKey|api-key|apikey|secret|token)"\s*:\s*"[a-zA-Z0-9_-]{20,}"/gi,
    '"$1": "[redacted]"',
  ],
...
export function isStreamingClient(
