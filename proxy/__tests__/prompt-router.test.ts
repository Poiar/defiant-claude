...
describe('classifyRequest', () => {
    ...
    test('CHAT: single long message that is not code', () => {
        const body = {
            messages: [{ role: 'user', content: 'Can you explain the difference between functional programming and object-oriented programming with some practical examples?' }],
        };
        expect(classifyRequest(body).tier).toBe('CHAT');
    });
});
...
describe('resolvePromptRoute', () => {
    ...
    test('TRIVIAL tier matches first route entry', () => {
        ...
        expect(result).toEqual({ providerKey: 'or', rewriteModel: 'liquid/lfm-2.5-1.2b-instruct:free' });
    });
